<template>
  <div>
    <div class="d-flex justify-content-start" style="gap: 10px">
      <span class="text-header">Agregar productos</span>
      <feather-icon
        name="plus-circle"
        size="24"
        class="text-primary"
    ...
    <div class="products-form">
      <validation-observer ref="formProduct">
        <b-row v-for="(product, index) in products" :key="index">
          <b-col cols="5">
            <validation-provider
              name="product_name"
              rules="required"
          ...
          <b-col cols="1">
            <feather-icon
              ...
              :clickable="true"
              v-b-tooltip.hover.top="'Remover producto'"
...
export default {
  ...
  methods: {
    ...
    async validateProductForm() {
      ...
      return validation;
    //   if (!validation) {
    //     return { isValid: false, message: "Hay productos con información incompleta" };
    //   }

    //   return { isValid: true, message: "Productos validados" };
    },
  },
};
</script>
